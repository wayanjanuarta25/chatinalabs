...
-- Version: 006_vector_search.sql
-- ==============================================================================

-- Enable pgvector extension if not already enabled
CREATE EXTENSION IF NOT EXISTS vector;

-- Create RPC function to match document chunks using cosine similarity
...
AS $$
BEGIN
  -- Workspace isolation security check for authenticated user
  IF auth.uid() IS NOT NULL AND target_workspace IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      RETURN; -- Deny access if user does not belong to target workspace
    END IF;
  END IF;
...
COMMENT ON FUNCTION match_document_chunks IS 'Performs pgvector cosine similarity search over document chunks filtered by workspace isolation.';
